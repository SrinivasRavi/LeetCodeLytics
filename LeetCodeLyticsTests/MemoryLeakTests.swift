...
/// Each test holds a `weak` reference, then drops the strong reference inside an
/// `autoreleasepool`; if the weak ref is still non-nil after, a retain cycle exists.
@MainActor
final class MemoryLeakTests: XCTestCase {
    ...
    func testDashboardViewModel_deallocatesAfterLoad() async {
        ...
        XCTAssertNil(weakVM, "DashboardViewModel must deallocate — possible retain cycle")
    }
    ...
    func testSubmissionsViewModel_deallocatesAfterLoad() async {
        ...
        // vm goes out of scope when the Task body exits; check after
        autoreleasepool {}
