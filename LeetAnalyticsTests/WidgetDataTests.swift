...
final class WidgetDataTests: XCTestCase {
    ...
    func testPlaceholder_isDCCAvailable_isFalse() {
        XCTAssertFalse(WidgetData.placeholder.isDCCAvailable)
    }
    ...
    func testEncodeDecode_roundTrip_preservesAllFields() throws {
        ...
        XCTAssertTrue(decoded.isDCCAvailable)
    ...
    func testEncodeDecode_isDCCAvailable_false_roundTrips() throws {
        let original = WidgetData(
        ...
        let encoded = try JSONEncoder().encode(original)
        let decoded = try JSONDecoder().decode(WidgetData.self, from: encoded)
        XCTAssertFalse(decoded.isDCCAvailable)
    }
    ...
    func testEncoded_recentCalendar_usesStringKeys() throws {
        let data = WidgetData(
            anysolveStreak: 0, dccStreak: 0, isDCCAvailable: true,
            easySolved: 0, mediumSolved: 0, hardSolved: 0,
    ...
    func testDecode_legacyData_withoutIsDCCAvailable_defaultsToTrue() throws {
        // Simulate data encoded before isDCCAvailable was added — must default to true
        // to preserve existing behaviour (don't show "–" on upgrade).
        let legacyJSON = """
        {"anysolveStreak":5,"dccStreak":3,"easySolved":10,"mediumSolved":5,"hardSolved":1,"recentCalendar":{}}
        """.data(using: .utf8)!
        let decoded = try JSONDecoder().decode(WidgetData.self, from: legacyJSON)
